...
describe('javascript-Recall', () => {

    it('selects elements starting with A', () => {
        let result = selectElementsStartingWithA(['bananas', 'apples', 'pears', 'avocados']);
        expect(result).toEqual(['apples', 'avocados']);
    });

    it('selects elements starting with a vowel', () => {
        let result = selectElementsStartingWithVowel(['john', 'david', 'omar', 'fred', 'idris', 'angela']);
        expect(result).toEqual(['omar', 'idris', 'angela']);
    });

    it('removes null values from an array', () => {
        let result = removeNullElements(['a', 'b', null, null, false, 0]);
        expect(result).toEqual(['a', 'b', false, 0]);
    });

    it('removes null and false values from an array', () => {
        let result = removeNullAndFalseElements(['a', 'b', null, null, false, 0]);
    ...
    it('takes all elements except the first 3', () => {
        let result = allElementsExceptFirstThree([1, 2, 3, 4, 5, 6, 7, 8]);
        expect(result).toEqual([4, 5, 6, 7, 8]);
    });

    it('adds an element to the beginning of an array', () => {
        let result = addElementToBeginning([2, 3, 4, 5], 1);
        expect(result).toEqual([1, 2, 3, 4, 5]);
    });

    it('sorts an array by the last letter of each word', () => {
        let result = sortByLastLetter(['Lannister', 'Stark', 'Greyjoy', 'Targaryen']);
        expect(result).toEqual(['Stark', 'Targaryen', 'Lannister', 'Greyjoy']);
    });

    it('gets the first half of a string', () => {
        let resultOne = getFirstHalf('dragon');
        let resultTwo = getFirstHalf('snake');
    ...
    it('makes numbers negative', () => {
        let resultOne = makeNegative(5);
        let resultTwo = makeNegative(-5);
    ...
    it('counts elements in an array that are palindromes', () => {
        let result = numberOfPalindromes(['kayak', 'noon', 'khaleesi', 'hodor', 'racecar', 'abcbc', 'abcba']);
        expect(result).toEqual(4);
    });

    it('returns the shortest word in an array', () => {
        let result = shortestWord(['winter', 'is', 'coming']);
        expect(result).toEqual('is');
    });

    it('returns the longest word in an array', () => {
        let result = longestWord(['A', 'Lannister', 'always', 'pays', 'his', 'debts'])
    ...
    it('turns a number string into a number', () => {
        let result = stringToNumber('3');
        expect(result).toEqual(3);
    });

    it('calculates the average of an array of numbers', () => {
        let result = calculateAverage([10, 15, 25]);
        expect(result).toEqual((10 + 15 + 25) / 3);
    });

    it('gets elements until greater than five', () => {
        let result = getElementsUntilGreaterThanFive([1, 3, 5, 4, 1, 2, 6, 2, 1, 3, 7]);
        expect(result).toEqual([1, 3, 5, 4, 1, 2]);
    });

    it('converts an array to an object', () => {
        let result = convertArrayToObject(['Jaime', 'Lannister', 'Robb', 'Stark', 'Joffrey', 'Baratheon']);
        expect(result).toEqual({
            'Jaime': 'Lannister',
            'Robb': 'Stark',
            'Joffrey': 'Baratheon'
    ...
    it('gets all letters in an array of words', () => {
        // returns sorted
        let result = getAllLetters(['cat', 'dog', 'fish']);
        expect(result).toEqual(['a', 'c', 'd', 'f', 'g', 'h', 'i', 'o', 's', 't']);
        // deals with repeated letters
        let result2 = getAllLetters(['cat', 'dog', 'fish', 'cat']);
    ...
    it('removes capital letters from a string', () => {
        let result = removeCapitals('Hello Kitty');
        expect(result).toEqual('ello itty');
    });

    it('rounds up a number', () => {
        let result = roundUp(4.3942);
    ...
    it('gets the domain name from an email address', () => {
        let result = getDomainName('spike@makersacademy.com');
        expect(result).toEqual('makersacademy');
        let result2 = getDomainName('spike@ssh.makersacademy.com');
        expect(result2).toEqual('ssh.makersacademy');
    });

    it('titleizes a string', () => {
        let result = titleize('the lion the witch and the wardrobe');
        expect(result).toEqual('The Lion the Witch and the Wardrobe');
        let result2 = titleize('the lion the witch. and the wardrobe');
        expect(result2).toEqual('The Lion the Witch. And the Wardrobe');
    });

    it('checks a string for special characters', () => {
        let resultOne = checkForSpecialCharacters('ABCdef123');
        let resultTwo = checkForSpecialCharacters('ABC@dsklfj!');
    ...
    it('finds the square root of a number', () => {
        let resultOne = squareRoot(9);
        let resultTwo = squareRoot(3);
    ...
    it('finds all possible anagrams of a word', () => {
        let result = findAnagrams('mad');
        let expectedAnagrams = ["adm", "amd", "dam", "dma", "mad", "mda"];
        for (let anagram of expectedAnagrams) {
            expect(result).toContain(anagram);
        }
        let result2 = findAnagrams('ma');
        let expectedAnagrams2 = ["am", "ma"];
    ...
    it('converts Fahrenheit to Celsius rounding to the nearest integer', () => {
        let resultOne = convertToCelsius(32);
        let resultTwo = convertToCelsius(55);
    ...
    it('changes each letter of an array into its position in the alphabet', () => {
        let result = letterPosition(['H', 'e', 'l', 'l', 'o', 'k', 'i', 't', 't', 'y']);
